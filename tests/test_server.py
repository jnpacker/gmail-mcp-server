...
from conftest import make_email, text, call_tool_sync, list_tools_sync
...
class TestResolveMessageIds:
    ...
    def test_combines_both(self):
        ids = self.srv._resolve_message_ids({"positions": [2], "message_ids": ["id-x"]})
...
class TestFormatEmailList:
    ...
    def test_position_map_built(self):
        emails = [make_email("m1", "A"), make_email("m2", "B")]
        self.srv._format_email_list(emails)
        assert self.srv.email_position_map == {1: "m1", 2: "m2"}

    def test_thread_grouping_header(self):
        emails = [
            make_email("m1", "Thread Subject", thread_id="t1"),
            make_email("m2", "Re: Thread Subject", thread_id="t1"),
        ]
        output = self.srv._format_email_list(emails)
    ...
    def test_no_thread_header_for_single(self):
        emails = [make_email("m1", "Solo Email", thread_id="t1")]
        output = self.srv._format_email_list(emails)
        assert "--- Thread:" not in output

    def test_user_labels_shown(self):
        emails = [make_email("m1", "Labeled", label_ids=["INBOX", "UNREAD", "Label_1"])]
        output = self.srv._format_email_list(emails)
        assert "Labels: Triage/Jira" in output

    def test_hidden_labels_filtered(self):
        emails = [make_email("m1", "X", label_ids=["INBOX", "UNREAD", "SPAM", "IMPORTANT"])]
        output = self.srv._format_email_list(emails)
        assert "Labels:" not in output

    def test_full_body_not_truncated(self):
        long_body = "A" * 500
        emails = [make_email("m1", "Long", body=long_body)]
        output = self.srv._format_email_list(emails)
        assert long_body in output
        assert "..." not in output

    def test_jira_body_cleaned(self):
        jira_body = "Real comment\n---\nThis message was sent by Atlassian Jira\n"
        emails = [make_email("m1", "[RH Jira] ACM-123 update", body=jira_body)]
        output = self.srv._format_email_list(emails)
    ...
    def test_jira_detected_by_ticket_pattern(self):
        jira_body = "Comment\n---\nThis message was sent by Atlassian Jira\n"
        emails = [make_email("m1", "ACM-456 something broke", body=jira_body)]
        output = self.srv._format_email_list(emails)
        assert "Atlassian Jira" not in output

    def test_non_jira_body_not_cleaned(self):
        body = "Some text\nA normal separator line\nMore content"
        emails = [make_email("m1", "Regular email", body=body)]
        output = self.srv._format_email_list(emails)
    ...
    def test_no_readable_content_hidden(self):
        emails = [make_email("m1", "Empty", body="No readable content")]
        output = self.srv._format_email_list(emails)
        assert "Body:" not in output

    def test_label_fetch_failure_graceful(self):
        self.srv.gmail_client.list_labels.side_effect = Exception("API error")
        emails = [make_email("m1", "Test", label_ids=["Label_1"])]
        output = self.srv._format_email_list(emails)
        assert "Labels: Label_1" in output

    def test_multiple_threads(self):
        emails = [
            make_email("m1", "Thread A msg 1", thread_id="t1"),
            make_email("m2", "Thread A msg 2", thread_id="t1"),
            make_email("m3", "Solo email", thread_id="t2"),
            make_email("m4", "Thread B msg 1", thread_id="t3"),
            make_email("m5", "Thread B msg 2", thread_id="t3"),
        ]
        output = self.srv._format_email_list(emails)
...
class TestToolHandlers:
    ...
    def _call(self, name, arguments=None):
        resp = call_tool_sync(self.srv, name, arguments)
        return resp.content
    ...
    def test_delete_emails_by_positions(self):
        ...
        t = text(result)
        assert "Deleted: Test" in t
        assert "Deleted: Test2" in t
    ...
    def test_delete_emails_failure(self):
        ...
        t = text(result)
        assert "Failed to delete id-a" in t
    ...
    def test_archive_emails_by_positions(self):
        ...
        t = text(result)
        assert "Archived: Archived Subj" in t
    ...
    def test_create_label(self):
        ...
        t = text(result)
        assert "Created label: Triage/New" in t
        assert "L3" in t
        assert len(self.srv.recent_actions) == 1
    ...
    def test_create_label_missing_name(self):
        ...
        t = text(result)
        assert "required" in t.lower() or "name" in t.lower()
    ...
    def test_modify_labels(self):
        ...
        result = self._call(
            "modify_labels",
            {"positions": [1], "add_labels": ["Triage/Jira"], "remove_labels": []},
        )
        t = text(result)
        assert "Labels modified: id-a" in t
