...
## Step 1.5: Fix Multi-Labeled Emails (Cleanup Pass)

Before classifying anything, scan every email returned by `list_unread_emails` for emails that already have **more than one `Triage/*` label**.

For each such email:
1. Re-read its subject, sender, and body to determine the single most appropriate `Triage/*` label
2. Issue **one** `modify_labels` call with:
   - `remove_labels`: all current `Triage/*` labels on that email
   - `add_labels`: the single correct label
3. Mark this email as already-labeled so Step 3b does not label it again (unless you decide to reassign it to a different group during classification — in which case Step 3b will handle it)

If no emails have multiple `Triage/*` labels, skip this step silently.
...
**CRITICAL**: Before making any `modify_labels` calls, build a complete final mapping of every email → its one assigned label. Each email must appear in this mapping exactly once. If you catch yourself assigning a second label to an email, discard the second assignment — the first one wins.

For each unique label needed:
1. Check if the label already exists (from Step 1 label list)
2. If not, call `create_label` to create it

Then for each email, make **exactly one** `modify_labels` call that does BOTH operations together:
- `remove_labels`: list all `Triage/*` labels currently on the email (from Step 1 data)
- `add_labels`: the single assigned `Triage/GroupName`

Never call `modify_labels` twice for the same email. Never add more than one `Triage/*` label in a single call.
...
Output ONLY the dashboard below — no preamble, commentary, or code fences. Start your response with ╔:
...
- **One label per email** — Each email gets exactly one `Triage/*` label. Build the complete email→label mapping before making any `modify_labels` calls. Each email may appear in only one `modify_labels` call total.
